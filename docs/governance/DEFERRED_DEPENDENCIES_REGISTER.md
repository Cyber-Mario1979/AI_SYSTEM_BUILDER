...
| DDR-005 | Standards embedding / retrieval index | Standards / retrieval | Standards retrieval/indexing can only be introduced after standards source/citation authority exists and cannot become source truth. | M17 retrieval-use governance; post-M20 gap assessment standards embedding readiness concern; `docs/milestones/M25/DDR_005_STANDARDS_EMBEDDING_RETRIEVAL_NAMED_DEFERRAL_DECISION.md` | Deferred | High | No for M25.2 named deferral; yes before standards embedding/retrieval implementation | Yes when standards-backed retrieval is productized | Phase 9 / M26 scope lock / standards-backed product behavior | Retrieval/index work, standards embedding, AI-assisted standards use, productized standards-backed retrieval | Indexing design; source registry dependency satisfied; retrieval-use rules; proof retrieval remains helper-only and not evidence authority; validation/evaluation evidence; UAT/acceptance where applicable | Depends on DDR-004 | Project Owner | M25.2 DDR-005 named deferral approval | M26.1 scope lock and M26.5-DDR-005 before standards embedding/retrieval implementation or productized standards-backed retrieval | Deferred to named checkpoint `M26.5-DDR-005` after DDR-004 closure. No standards embedding/retrieval implementation is authorized during M25. Must remain retrieval helper, not evidence authority. |
...
| DDR-007 | Actual model/provider integration and pre-go-live operational testing path | AI/runtime/product readiness | Live model/provider calls and pre-go-live operational testing require a roadmap-authorized path, provider adapter boundary, smoke tests, operational test plan, local heavy-use / operational shakedown gate, validation evidence, and UAT / acceptance evidence before product/SaaS-facing use. | Issue #16; M20 closeout exclusions; post-M20 gap assessment G-06; `docs/milestones/M25/DDR_007_MODEL_PROVIDER_INTEGRATION_AND_LOCAL_SHAKEDOWN_PLACEMENT_DECISION.md` | Closure Planned | Critical | No for M25.2 placement; yes before any live model/provider implementation or pre-go-live execution | Yes | Phase 9 / M26 scope lock / M27 go-no-go / pre-go-live | Model/provider integration, live AI runtime, deployment readiness, operational testing, local heavy-use pilot, SaaS go-live | Provider strategy decision; provider adapter boundary; smoke tests; operational test plan; local heavy-use / operational shakedown protocol and evidence; issue/iteration evidence; validation evidence; UAT / acceptance evidence | Issue #16 | Project Owner | M25.2 DDR-007 placement approval | M26.1 scope lock and before any provider adapter implementation, local shakedown, pre-go-live execution, or SaaS go-live | Closure planned for placement only. Product/SaaS-facing live model/provider calls remain blocked. Controlled local/pilot heavy-use testing is allowed only after roadmap-authorized provider boundary and test planning exist. Local/offline model support may be considered later but is not assumed by this placement decision. |
| DDR-008 | Phase 8 / Phase 9 productization readiness gate | Productization / deployment / SaaS | Phase 8 and Phase 9 required detailed expansion plus dependency review before execution; this gate-control dependency is now satisfied for Phase 9 entry because detailed Phase 9 checkpoint authority and M25.2 DDR disposition evidence exist. | Roadmap continuation Part 2; Issue #16; post-M20 gap assessment; `ROADMAP_ADDENDUM_10_PHASE_9_DETAILED_CHECKPOINT_LADDER.md`; `docs/milestones/M25/M25_1_PRODUCTIZATION_BOUNDARY_ASSESSMENT.md`; `docs/milestones/M25/M25_2_DEFERRED_DEPENDENCY_DISPOSITION_REVIEW.md`; `docs/milestones/M25/M25_2_DDR_CLOSURE_PLAN.md`; `docs/milestones/M25/DDR_008_PHASE_8_9_READINESS_GATE_CLOSURE_DECISION.md` | Closed | Critical | No for Phase 8 / Phase 9 readiness gate-control scope | No for gate-control closure; downstream productization blockers remain governed by their own DDRs | Phase 9 / M25.2 gate-control closure | Phase transition, cloud/compute/deployment planning, SaaS planning, productization readiness review | Phase expansion addendum; full DDR review; explicit unresolved-dependency disposition; gate closure decision | Issue #16 related | Project Owner | M25.2 DDR-008 closure approval | None unless Phase 9 readiness gate is reopened or new productization phase-gate ambiguity appears | Closed for gate-control scope only. Closure does not claim product readiness, SaaS readiness, Phase 9 completion, or downstream blocker closure. Productization remains blocked where DDR-005, DDR-006, DDR-007, or M26-scoped executable implementation dependencies apply. |
| DDR-009 | External contract placeholders for future library/template/standards references | UI/API external contracts | M21 external contracts preserved compatibility with future references such as `template_id`, `schema_id`, `standards_bundle_ref`, `citation_ref`, and `library_version` without implementing those libraries or pretending the downstream dependencies are closed. | M21.1 roadmap scope; post-M20 gap assessment; DDR-001 through DDR-006; `docs/milestones/M21/M21_1_SHARED_EXTERNAL_CONTRACT_DISCIPLINE.md`; `asbp/external_surface/contracts.py`; `tests/test_external_surface_contracts.py`; `docs/milestones/M25/DDR_009_EXTERNAL_CONTRACT_PLACEHOLDER_EVIDENCE_VERIFICATION_DECISION.md` | Closed | High | No for M21 placeholder compatibility | No for placeholder compatibility; underlying productized behavior remains governed by related DDRs | Historical M21 placeholder compatibility / Phase 9 productization awareness | External contract changes, productized placeholder behavior, library/template/standards reference implementation | M21 contract docs/tests showing placeholders or extension points without pretending dependencies are closed | None yet | Project Owner | M25.2 DDR-009 evidence verification approval | None unless M21 external contract placeholders are modified or productized placeholder behavior is proposed | Closed for M21 external contract placeholder compatibility evidence. Closure does not implement or authorize runtime libraries, product-ready templates, standards embedding/retrieval, citation authority, product-ready generation, or productized placeholder behavior. |
...
As of M25.2 DDR-008 readiness gate-control closure approval:
...
- `DDR-005` is deferred to named checkpoint `M26.5-DDR-005`; standards embedding/retrieval implementation remains blocked until M26 scope, closure evidence, validation/evaluation evidence, and acceptance evidence exist or the dependency is formally reclassified.
- `DDR-009` is `Closed` for M21 external contract placeholder compatibility evidence. This closure does not implement or authorize productized library/template/standards placeholder behavior.

- `DDR-008` is `Closed` for Phase 8 / Phase 9 readiness gate-control scope. Closure confirms Phase 9 has detailed checkpoint authority and M25.2 DDR disposition evidence; it does not claim product readiness, SaaS readiness, or downstream blocker closure.
...
- Productization must not proceed where `DDR-005`, `DDR-006`, `DDR-007`, or any M26-scoped executable implementation dependency remains a blocker for the affected behavior; `DDR-009` closure does not remove those underlying blockers.

- `DDR-008` closure does not remove the underlying blockers controlled by `DDR-005`, `DDR-006`, `DDR-007`, or M26-scoped executable implementation decisions.
- Standards embedding/retrieval remains governed by `DDR-005`, is deferred to `M26.5-DDR-005`, and requires roadmap/checkpoint authority plus validation/evaluation evidence before implementation or productized use.
- DDR-007 is `Closure Planned` for placement only. Actual model/provider integration must not proceed without a roadmap-authorized path, provider adapter boundary, smoke tests, operational test plan, local heavy-use / operational shakedown evidence, validation evidence, and UAT / acceptance evidence.
- No executable validation is required for the DDR-003 model closure, DDR-006 scope/closure-path approval, DDR-007 placement approval, DDR-005 named deferral approval, DDR-009 evidence verification approval, or DDR-008 readiness gate-control closure because they are documentation/governance-only and do not alter code, tests, commands, imports, runtime behavior, CLI behavior, or executable contracts.
