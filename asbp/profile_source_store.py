...
def list_profiles_by_preset_family(
    library: ProfileLibraryModel,
    preset_family: PresetFamilyId,
) -> list[ProfileSourceModel]:
    return [
        profile
        for profile in library.profiles
        if preset_family in profile.related_preset_families
    ]


def get_profile_context_field_by_id(
    profile: ProfileSourceModel,
    field_id: str,
) -> ProfileContextFieldModel:
    ...
    raise ValueError(
        "Profile context field definition not found: "
        f"{profile.profile_id}::{field_id}"
    )


def build_profile_context_definition_id(
    profile_id: str,
    field_id: str,
) -> str:
