...
PlanningBasisSourceStatus = Literal["starter_runtime_source"]
DurationEstimateType = Literal[
    "starter_estimate",
    "human_confirmed_estimate",
    "site_defined_estimate",
    "future_mapping_expected",
]
DurationUnit = Literal["working_day"]
EstimationSourceStatus = Literal[
    "starter_baseline",
    "sme_estimate_required",
    "site_input_required",
    "future_confirmation_required",
]
ScopeComplexity = Literal[
    "simple",
    "standard",
    "complex",
    "not_applicable",
]
CalendarDependencyStatus = Literal[
    "calendar_aware_no_capacity_calculation",
    "calendar_required_before_scheduling",
    "not_applicable",
]
...
class DurationRangeModel(BaseModel):
    ...
    def validate_duration_order(self):
        if not self.min_days <= self.typical_days <= self.max_days:
            raise ValueError(
                "Duration range must satisfy min_days <= typical_days <= max_days"
            )
        return self
...
class DurationSourceModel(BaseModel):
    ...
    def validate_duration_source_boundary(self):
        if (
            self.estimate_type == "future_mapping_expected"
            and self.duration_range.typical_days > 0
        ):
            raise ValueError(
                "future_mapping_expected duration source cannot include a "
                f"non-zero typical estimate: {self.duration_ref_id}"
            )

        link_ids: set[tuple[str, str]] = set()
        for linked_task_source in self.linked_task_sources:
            link_id = (
                linked_task_source.task_pool_id,
                linked_task_source.atomic_task_id,
            )
            if link_id in link_ids:
                raise ValueError(
                    "Duplicate linked task source is not allowed for "
                    f"{self.duration_ref_id}: "
                    f"{linked_task_source.task_pool_id}::"
                    f"{linked_task_source.atomic_task_id}"
                )
    ...
    def validate_unique_duration_refs(self):
        ...
        for duration_source in self.duration_sources:
            if duration_source.duration_ref_id in duration_ref_ids:
                raise ValueError(
                    f"Duplicate duration_ref_id is not allowed: "
                    f"{duration_source.duration_ref_id}"
                )
...
def collect_planning_basis_duration_refs(
