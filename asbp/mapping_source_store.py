...
from asbp.mapping_source_model import (
    MappingKind,
    MappingLibraryModel,
    MappingSourceModel,
    ReferenceType,
...
def list_mappings_by_kind(
    library: MappingLibraryModel,
    mapping_kind: MappingKind,
) -> list[MappingSourceModel]:
    return [
        mapping
        for mapping in library.mappings
...
def collect_resolved_reference_ids_by_type(
    library: MappingLibraryModel,
...
def find_missing_resolved_reference_ids(
    library: MappingLibraryModel,
    reference_type: ReferenceType,
    known_reference_ids: set[str],
) -> list[str]:
    mapped_reference_ids = collect_resolved_reference_ids_by_type(
        library,
        reference_type,
    )
...
def assert_resolved_references_exist(
    library: MappingLibraryModel,
    reference_type: ReferenceType,
    known_reference_ids: set[str],
) -> None:
    missing_reference_ids = find_missing_resolved_reference_ids(
        library,
        reference_type,
        known_reference_ids,
    )
    if missing_reference_ids:
        ...
        raise ValueError(
            f"Mapping references unresolved {reference_type} IDs: "
            f"{joined_missing_ids}"
        )


def build_mapping_reference_definition_id(
    mapping_id: str,
    reference_type: ReferenceType,
    reference_id: str,
) -> str:
