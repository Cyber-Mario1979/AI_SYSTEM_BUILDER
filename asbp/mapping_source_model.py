...
MappingSourceStatus = Literal["starter_runtime_source", "mvp_remediation_source"]
MappingKind = Literal["preset_to_profile", "selector_to_task_pool", "task_to_document", "standard_to_template"]
...
ReferenceStatus = Literal["resolved_source", "future_expected", "placeholder_not_runtime_authority"]
...
class MappingReferenceModel(BaseModel):
    ...
    def validate_future_or_placeholder_resolution(self):
        if self.reference_status != "resolved_source" and self.resolution_checkpoint is None:
            raise ValueError("Future or placeholder mapping reference requires " f"resolution_checkpoint: {self.reference_id}")
        if self.reference_status == "resolved_source" and self.resolution_checkpoint is not None:
            raise ValueError("Resolved mapping reference cannot include " f"resolution_checkpoint: {self.reference_id}")
        return self
...
class MappingSourceModel(BaseModel):
    ...
    mapping_id: str = Field(min_length=1, pattern=r"^MAP-[A-Z0-9-]+@v[0-9]+$")
    ...
    def _validate_no_duplicate_references(self) -> None:
        ...
        for reference in [*self.source_refs, *self.target_refs]:
            ...
            if reference_key in seen_refs:
                raise ValueError(f"Duplicate mapping reference is not allowed in {self.mapping_id}: {reference.reference_type}::{reference.reference_id}")
    ...
    def _validate_reference_types_by_kind(self) -> None:
        ...
        allowed_source_types, allowed_target_types = expected_types[self.mapping_kind]
        for reference in self.source_refs:
            if reference.reference_type not in allowed_source_types:
                raise ValueError(f"{self.mapping_kind} mapping has invalid source reference_type: {reference.reference_type}")
        for reference in self.target_refs:
            if reference.reference_type not in allowed_target_types:
                raise ValueError(f"{self.mapping_kind} mapping has invalid target reference_type: {reference.reference_type}")

    def _validate_reference_status_by_kind(self) -> None:
        if self.mapping_kind in {"preset_to_profile", "selector_to_task_pool"}:
            for reference in [*self.source_refs, *self.target_refs]:
                if reference.reference_status != "resolved_source":
                    raise ValueError(f"{self.mapping_kind} mapping requires resolved_source references: {reference.reference_id}")
        if self.mapping_kind == "task_to_document":
            for reference in self.source_refs:
                if reference.reference_status != "resolved_source":
                    raise ValueError("task_to_document source atomic_task must be resolved_source: " f"{reference.reference_id}")
            for reference in self.target_refs:
                if reference.reference_status == "resolved_source":
                    raise ValueError("task_to_document document targets must remain future or placeholder references in remediation: " f"{reference.reference_id}")
        if self.mapping_kind == "standard_to_template":
            for reference in self.source_refs:
                if reference.reference_status != "resolved_source":
                    raise ValueError("standard_to_template source standard_bundle must be resolved_source after M28.4: " f"{reference.reference_id}")
            for reference in self.target_refs:
                if reference.reference_status != "resolved_source":
                    raise ValueError("standard_to_template template targets must be resolved_source after M29.2 template record implementation: " f"{reference.reference_id}")
    ...
    def validate_unique_mapping_ids(self):
        ...
        for mapping in self.mappings:
            if mapping.mapping_id in mapping_ids:
                raise ValueError(f"Duplicate mapping_id is not allowed: {mapping.mapping_id}")
