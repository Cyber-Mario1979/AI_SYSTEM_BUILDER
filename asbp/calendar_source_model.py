...
CalendarSourceStatus = Literal["starter_runtime_source"]
...
HolidayRuleSourceStatus = Literal[
    "human_input_required",
    "starter_unverified",
    "not_applicable",
]
WorkmonthBasis = Literal[
    "calendar_month",
    "user_defined_period",
    "site_defined_period",
]
    ...
    def validate_working_day_times(self):
        if self.is_working_day:
            if self.start_time is None or self.end_time is None:
                raise ValueError(
                    f"Working day requires start_time and end_time: {self.day}"
                )
            if _time_to_minutes(self.start_time) >= _time_to_minutes(self.end_time):
                raise ValueError(
                    f"Working day start_time must be before end_time: {self.day}"
                )
        else:
            if self.start_time is not None or self.end_time is not None:
                raise ValueError(
                    f"Non-working day cannot include work times: {self.day}"
                )
            if self.break_minutes != 0:
                raise ValueError(
                    f"Non-working day cannot include break_minutes: {self.day}"
                )
...
class CalendarSourceModel(BaseModel):
    ...
    def validate_calendar_scope(self):
        ...
        elif self.calendar_type == "workmonth":
            if not self.workmonth_rules:
                raise ValueError(
                    f"workmonth calendar requires workmonth_rules: "
                    f"{self.calendar_id}"
                )
        elif self.calendar_type == "holiday_policy":
            if not self.holiday_rules:
                raise ValueError(
                    f"holiday_policy calendar requires holiday_rules: "
                    f"{self.calendar_id}"
                )
        elif self.calendar_type == "manual_fallback":
            if self.regional_assumption_status != "human_confirmation_required":
                raise ValueError(
                    "manual_fallback calendar requires "
                    "human_confirmation_required regional_assumption_status: "
                    f"{self.calendar_id}"
                )
    ...
    def _validate_visible_regional_assumption(self) -> None:
        ...
        if (
            ("cairo" in regional_text or "egypt" in regional_text)
            and self.regional_assumption_status == "not_applicable"
        ):
            raise ValueError(
                "Cairo/Egypt calendar context must use an explicit regional "
                f"assumption status: {self.calendar_id}"
            )
    ...
    def _validate_unique_nested_ids(self) -> None:
        ...
        for work_day in self.working_days:
            if work_day.day in seen_days:
                raise ValueError(
                    f"Duplicate working day is not allowed in "
                    f"{self.calendar_id}: {work_day.day}"
                )
        ...
        for rule in self.workmonth_rules:
            if rule.rule_id in seen_workmonth_rules:
                raise ValueError(
                    f"Duplicate workmonth rule_id is not allowed in "
                    f"{self.calendar_id}: {rule.rule_id}"
                )
        ...
        for rule in self.holiday_rules:
            if rule.rule_id in seen_holiday_rules:
                raise ValueError(
                    f"Duplicate holiday rule_id is not allowed in "
                    f"{self.calendar_id}: {rule.rule_id}"
                )
    ...
    def _validate_workweek(self) -> None:
        ...
        if actual_days != expected_days:
            raise ValueError(
                f"workweek calendar must define all seven weekdays: "
                f"{self.calendar_id}"
            )
    ...
    def validate_unique_calendar_ids(self):
        ...
        for calendar in self.calendars:
            if calendar.calendar_id in calendar_ids:
                raise ValueError(
                    f"Duplicate calendar_id is not allowed: "
