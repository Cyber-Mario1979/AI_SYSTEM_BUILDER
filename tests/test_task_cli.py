...
def test_wp_update_status_updates_only_existing_work_package_status(restore_state_file):
    STATE_FILE.parent.mkdir(parents=True, exist_ok=True)
    STATE_FILE.write_text(
        json.dumps(
            {
    ...
    result = run_cli("wp", "update-status", "WP-002", "completed")

    assert result.returncode == 0
    assert "Work Package status updated: WP-002 -> completed" in result.stdout
...
def test_wp_update_status_handles_missing_work_package_id(restore_state_file):
    STATE_FILE.parent.mkdir(parents=True, exist_ok=True)
    STATE_FILE.write_text(
        json.dumps(
            {
    ...
    result = run_cli("wp", "update-status", "WP-999", "completed")
...
def test_wp_update_status_handles_missing_state_file(restore_state_file):
    if STATE_FILE.exists():
        STATE_FILE.unlink()

    result = run_cli("wp", "update-status", "WP-001", "completed")
...
def test_wp_update_status_rejects_invalid_choice_at_parser_level():
    result = run_cli("wp", "update-status", "WP-001", "planned")
