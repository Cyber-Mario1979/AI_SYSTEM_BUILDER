...
def test_runtime_handoff_wp_cli_returns_handoff_payload(restore_state_file):
    STATE_FILE.parent.mkdir(parents=True, exist_ok=True)
    ...
    result = run_cli("runtime", "handoff-wp", "WP-001")

    assert result.returncode == 0
    payload = json.loads(result.stdout)
    assert payload["wp_id"] == "WP-001"
    assert payload["handoff_metadata"]["handoff_contract_id"] == (
        "work_package_llm_handoff_v1"
    )
    assert payload["handoff_metadata"]["handoff_state"] == "blocked"


def test_runtime_handoff_wp_cli_reports_missing_work_package(restore_state_file):
    STATE_FILE.parent.mkdir(parents=True, exist_ok=True)
    STATE_FILE.write_text(
        json.dumps(
            {
    ...
    result = run_cli("runtime", "handoff-wp", "WP-404")
