...
M23_DOC_PATH = ROOT / "docs" / "milestones" / "M23" / "M23_1_DEPLOYMENT_BOUNDARY_FOUNDATION.md"
TRACKER_PATH = ROOT / "PROGRESS_TRACKER.md"

M23_DOC_CONTENT = """# M23_1_DEPLOYMENT_BOUNDARY_FOUNDATION
...
def update_tracker() -> None:
    if not TRACKER_PATH.exists():
        fail("PROGRESS_TRACKER.md not found. Run this script from the repository root.")
    ...
    for old, new in replacements:
        if old not in text:
            fail(f"Expected tracker text not found for replacement:\n{old}")
        text = text.replace(old, new, 1)

    TRACKER_PATH.write_text(text, encoding="utf-8")

def write_m23_doc() -> None:
    if M23_DOC_PATH.exists():
        fail(f"{M23_DOC_PATH.relative_to(ROOT)} already exists. Refusing to overwrite.")

    M23_DOC_PATH.parent.mkdir(parents=True, exist_ok=True)
    M23_DOC_PATH.write_text(M23_DOC_CONTENT, encoding="utf-8")

def main() -> None:
    write_m23_doc()
    update_tracker()

    print("Applied M23.1 user-applied package:")
    print(f"- Created {M23_DOC_PATH.relative_to(ROOT)}")
    print("- Updated PROGRESS_TRACKER.md")
    print("")
    print("Recommended commit message:")
    print("docs: record M23.1 deployment boundary foundation")
